...
//open the dummy payment UI and send request to server to create a payment order for a booked slot
export const asynccheckOutBookSlot = createAsyncThunk('payments/checkoutBookSlot', async (param) => {
  try {
    const { formData, userData } = param;
    
    // For public parking lots, no payment needed
    if (formData.type === 'public') {
      const { data } = await checkoutBookSlot(formData);
      return { alertData: { msg: data.msg, type: 'success' } };
    }

    // For private parking lots, create order and return it for dummy payment
    const { data } = await checkoutBookSlot(formData);
    const { order } = data;

    // Return the order data - the component will handle opening the dummy payment modal
    return { 
      alertData: { msg: "Order created successfully", type: 'success' }, 
      order,
      formData 
    };
  } catch (err) {
