const { instance, verifyDummySignature, generateDummySignature } = require('../Utils/razorPayInstance')
...
exports.bookPaymentVerification = async (req, res) => {
    ...
    try {
        ...
        // Use dummy signature verification
        const isValidSignature = verifyDummySignature(razorpay_order_id, razorpay_payment_id, razorpay_signature);
        
        console.log('🔐 Dummy signature verification:', isValidSignature ? '✅ Valid' : '❌ Invalid');

        //if signature is valid, payment is successful
        if (isValidSignature) {
            //fetch the details of payment using dummy api
            const paymentDetails = await instance.payments.fetch(razorpay_payment_id)
            console.log("✅ Got dummy payment details")

            //update the bookedSlot with orderID as this with paid as true and also save the paymentDetails
            const bookedTimeSlot = await BookedTimeSlot.findOneAndUpdate(
                { orderID: razorpay_order_id }, 
                { paid: true, paymentDetails: paymentDetails }, 
                { new: true }
            )
            
            console.log("✅ Updated booked time slot")
            ...
            console.log("✅ Got parking lot details")
        ...
        //if signature is invalid, payment failed
...
//tested 
/*send the dummy razorpay key to frontend*/
exports.getRazorPayKey = async (req, res) => {
    try {
        const { DUMMY_KEY_ID } = require('../Utils/razorPayInstance')
        console.log("🔑 Sending dummy Razorpay key to frontend")
        return res.status(200).json({ key: DUMMY_KEY_ID })
    } catch (err) {
